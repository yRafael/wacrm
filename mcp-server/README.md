# Fire Workspace MCP server

A [Model Context Protocol](https://modelcontextprotocol.io) server for
**[Fire Workspace](https://github.com/yRafael/fire-workspace)** — the
self-hostable WhatsApp CRM. It lets MCP clients (Claude Desktop, Claude
Code, Cursor, and others) drive your CRM in natural language:
...
It's a thin wrapper over Fire Workspace's public
[`/api/v1`](../docs/public-api.md) REST API. All auth, scoping, and rate
limiting are enforced by your Fire Workspace instance — this server just
exposes the API as MCP tools.

## Prerequisites

1. A running Fire Workspace instance (your own self-hosted deploy).
...
| ------------------------ | -------- | ------------------------------------------------------------- |
| `FIRE_BASE_URL`          | yes      | Your instance URL, e.g. `https://crm.example.com`             |
| `FIRE_API_KEY`           | yes      | An API key from the dashboard                                 |
| `FIRE_ENABLE_WRITES`     | no       | `true` to expose contact writes + message sending             |
| `FIRE_ENABLE_BROADCASTS` | no       | `true` to expose mass broadcasts (needs `FIRE_ENABLE_WRITES`) |
...
```jsonc
{
  "mcpServers": {
    "fire-workspace": {
      "command": "npx",
      "args": ["-y", "fire-workspace-mcp"],
      "env": {
        "FIRE_BASE_URL": "https://crm.example.com",
        "FIRE_API_KEY": "fire_live_xxxxxxxxxxxxxxxxxxxxxxxx",
      },
    },
  },
}
...
"env": {
  "FIRE_BASE_URL": "https://crm.example.com",
  "FIRE_API_KEY": "fire_live_xxxxxxxxxxxxxxxxxxxxxxxx",
  "FIRE_ENABLE_WRITES": "true",
  "FIRE_ENABLE_BROADCASTS": "true"
}
```
...
1. **Read-only by default.** Write and broadcast tools are not even
   ...
   `FIRE_ENABLE_WRITES` / `FIRE_ENABLE_BROADCASTS`.
2. **API-key scopes.** Whatever the guards allow, your Fire Workspace
