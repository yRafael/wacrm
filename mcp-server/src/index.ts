#!/usr/bin/env node
// ============================================================
// Fire Workspace MCP server — entry point.
//
// A stdio Model Context Protocol server that exposes the Fire Workspace
...
import { FireClient } from './client.js';
...
async function main(): Promise<void> {
  ...
  const client = new FireClient(config);

  const server = new McpServer({
    name: 'fire-workspace-mcp',
    version: VERSION,
  });
  ...
  console.error(
    `Fire Workspace MCP server v${VERSION} ready — instance ${config.baseUrl}, ` +
      `tool groups: ${groups.join(', ')}` +
      (config.enableWrites
        ? ''
        : ' (read-only; set FIRE_ENABLE_WRITES to allow changes)')
  );
}

main().catch((err) => {
  console.error(
    `Failed to start Fire Workspace MCP server: ${(err as Error).message}`
  );
  process.exit(1);
});
