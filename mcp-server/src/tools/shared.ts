...
// layer dumb). On a FireApiError we return an `isError` result with
...
import { FireApiError } from '../client.js';
...
/**
 * Wrap a tool handler so any FireApiError becomes a clean, model-
  ...
  return async (args: A) => {
    try {
      ...
      if (err instanceof FireApiError) {
        return errorResult(
          `Fire Workspace API error [${err.code}]: ${err.message}`
        );
      }
