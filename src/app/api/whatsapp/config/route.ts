...
// NOTE: supabaseAdmin is now imported from '@/lib/supabase/admin'.
...
export async function GET() {
  try {
    // Viewing WhatsApp config triggers Meta API verification —
    // only admins should be able to do this, not viewers.
    let ctx;
    try {
    ...
    try {
      ...
      console.error('[whatsapp/config GET] Token decryption failed:', err instanceof Error ? err.message : 'unknown');
      return NextResponse.json(
        {
          connected: false,
...
export async function POST(request: Request) {
  try {
    // Saving the account's WhatsApp credentials is a settings-level write
    // (registers the phone number with Meta) — require `admin`.
    let ctx;
    try {
      ctx = await requireRole('admin');
    } catch (err) {
    ...
    // Fire Workspace is single-tenant-per-WhatsApp-number — letting two
    // accounts bind the same number causes the webhook's `.single()`
    // lookup to throw PGRST116 ("multiple rows"), silently dropping
    // every inbound message. Post-multi-user we key on account_id (not
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one Fire Workspace account.',
        },
        { status: 409 }
      );
    }
...
export async function DELETE() {
  try {
    // Removing the account's WhatsApp config is a settings-level
    // destructive write — require `admin`.
    let ctx;
    try {
      ctx = await requireRole('admin');
    } catch (err) {
