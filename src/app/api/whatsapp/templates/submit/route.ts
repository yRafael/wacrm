...
export async function POST(request: Request) {
  try {
    // Submitting a template pushes it to Meta with the account's WABA
    // credentials — require `admin` (settings-level capability).
    let ctx;
    try {
    ...
    if (upsertErr) {
      ...
      console.error('[templates/submit] local persist failed:', upsertErr);
      return NextResponse.json(
        {
          error:
