...
export async function POST(request: Request) {
  try {
    ...
    const limit = checkRateLimit(`parser:${user.id}`, RATE_LIMITS.parser)
    if (!limit.success) {
    ...
    if (panelType && !PANEL_TYPES.includes(panelType as (typeof PANEL_TYPES)[number])) {
      return NextResponse.json(
    ...
    if (credentialError || !credentialId) {
      console.error('Error saving IPTV credential:', credentialError?.message)
      return NextResponse.json(
    ...
    console.error('Error in IPTV parser save POST:', error)
    return NextResponse.json(
      { error: 'Failed to save credential' },
      { status: 500 },
    )
