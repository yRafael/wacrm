...
Link M3U HLS:
http://panel.example.com/get.php?username=00000000&password=11111111&type=m3u_plus&output=hls

*Att.:* fire tv`;

// A promotional broadcast with an indication= link but NO credentials.
const PROMO_MSG = `╭── *INDIQUE PARA UM AMIGO!*
├● 📡 ➤ https://referral.example.com/t/sample?indication=sample-code
  ...
  function builder(table: string) {
    ...
    const updateResult = () => {
      switch (table) {
        case 'iptv_credentials':
          return {
            data: { id: existingCredential?.id ?? 'cred-new' },
            error: null,
          };
        default:
          return { data: null, error: null };
    ...
    const terminal = () =>
      Promise.resolve(
        didUpdate ? updateResult() : didInsert ? insertResult() : selectResult()
      );
...
describe('POST /api/iptv/parser/save', () => {
  ...
  it('encrypts the password and logs the parse without duplicating it', async () => {
    ...
    expect(cred).toMatchObject({
      ...
      contact_id: 'contact-1',
      username: '00000000',
      password: 'enc:11111111',
      status: 'active',
    });
    expect(cred.password).not.toBe('11111111');
    ...
    expect(JSON.stringify(log.parsed_fields)).not.toContain('11111111');
  });
  ...
  it('logs a failed parse (promo text) but still saves the confirmed fields', async () => {
    ...
    expect(parserLogInserts).toHaveLength(1);
    expect(parserLogInserts[0]).toMatchObject({
      confidence: 0,
      status: 'error',
    });
