...
const getAccountStatus = vi.fn<(accountId: string) => Promise<string | null>>();
const touchLastUsed = vi.fn();
vi.mock('@/lib/api-keys/store', () => ({
  findActiveKeyByHash: (hash: string) => findActiveKeyByHash(hash),
  getAccountStatus: (accountId: string) => getAccountStatus(accountId),
  touchLastUsed: (id: string) => touchLastUsed(id),
}));

// Mock subscription gating — default to active (no blocking).
...
describe('requireApiKey', () => {
  ...
  it("401s on a token that doesn't look like a Fire Workspace key", async () => {
  ...
  it('403s when the key account is SUSPENDED (status gate §5.7)', async () => {
    findActiveKeyByHash.mockResolvedValue(row());
  ...
  it('403s when the key account is BANNED (status gate §5.7)', async () => {
    findActiveKeyByHash.mockResolvedValue(row());
    getAccountStatus.mockResolvedValue('BANNED');
    await expectApiError(
      requireApiKey(reqWith(`Bearer ${KEY}`)),
      'forbidden',
      403
