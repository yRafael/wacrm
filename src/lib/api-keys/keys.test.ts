...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('fire_live_abc')).toBe(hashApiKey('fire_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('fire_live_abc')).not.toBe(hashApiKey('fire_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('fire_live_xyz');
