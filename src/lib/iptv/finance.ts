...
export function totalRevenue(txns: FinancialTransaction[], range: DateRange = {}): number {
  return sumByType(txns, 'income', range);
}

/** Total cash-out: expenses + refunds (money leaving the business). */
export function totalExpenses(txns: FinancialTransaction[], range: DateRange = {}): number {
 ...
 */
export function netCashFlow(txns: FinancialTransaction[], range: DateRange = {}): number {
...
export function averageTicket(
  ...
  const pool = opts.paidOnly === false
    ? payments
...
export function delinquency(payments: Payment[], now: Date): Delinquency {
  ...
  const overdue = open.filter((p) => new Date(p.due_at).getTime() < now.getTime());
...
export function revenueByCategory(txns: FinancialTransaction[]): Record<string, number> {
...
export function monthlySeries(
  ...
  for (let i = 0; i < months; i++) {
    const start = new Date(cursor.getFullYear(), cursor.getMonth() - (months - 1 - i), 1);
