...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'nossa conta Fire',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Fire Workspace
    // account" wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'nossa conta Fire';
          ...
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              {/* Anchor styled with `buttonVariants` rather than wrapping
                  in <Button asChild>. The Fire Button is the Base UI
