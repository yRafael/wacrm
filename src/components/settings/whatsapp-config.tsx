...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error(
          'Digite novamente o Access Token para salvar as alterações'
        );
        setSaving(false);
        return;
      }
  ...
  async function handleTestConnection() {
    try {
      ...
      setConnectionStatus('disconnected');
      toast.error('A conexão falhou. Verifique a rede e tente novamente.');
    } finally {
      setTesting(false);
    }
  }
  ...
  async function handleVerifyRegistration() {
    ...
    try {
      ...
      if (data.live) {
        toast.success(
          'O número está totalmente configurado — a Meta está entregando os eventos.'
        );
      } else {
        toast.error(
  ...
  async function handleReset() {
    if (
      !confirm(
        'Isso excluirá a configuração atual do WhatsApp para que você possa reinseri-la. Continuar?'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.success(
        'Configuração limpa. Agora você pode reinserir suas credenciais.'
      );
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
