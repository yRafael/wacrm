...
export function MessageThread({
  ...
  // Client stats for the header pills + the ⚡ Ações / Plano widgets —
  // refetch whenever the thread's contact changes (mount) or after a
  // quick action write (onStatsChanged). Deliberately NOT keyed to
  // resyncToken: this is a 5-query aggregation and a tab-focus/reconnect
  // resync doesn't need a fresh copy (the contact didn't move), it would
  // just add load.
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : 'erro de rede';
        toast.error(`Falha ao enviar reação: ${reason}`);
