...
const METHODS: PaymentMethod[] = ["pix", "cash", "card", "transfer", "boleto", "credit"];
...
const AGENDA_KEYS: AgendaKey[] = ["overdue", "today", "tomorrow", "week", "month"];
    ...
    return (
      <section key={key} className="space-y-2">
        <div className="flex items-center gap-2">
          {icon}
          <h3 className="text-sm font-semibold text-foreground">{t(`agenda.${key}`)}</h3>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-foreground">{t("agendaTitle")}</h2>
          <p className="text-sm text-muted-foreground">{t("agendaSub")}</p>
        </div>
        <GatedButton
          canAct={canAct}
          gateReason={t("noPermission")}
            ...
            {renewals.map((r) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatDue(r.created_at)} · {t("expiresAt")} {formatDue(r.new_expires_at)}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  <Badge variant="outline">{t(`renewalType.${r.renewal_type}`)}</Badge>
                  <span className="text-sm font-semibold tabular-nums text-foreground">
      ...
      <Dialog open={showNewPayment} onOpenChange={setShowNewPayment}>
        <DialogContent className="max-h-[85vh] overflow-y-auto border-border bg-popover text-popover-foreground sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-popover-foreground">{t("registerPayment")}</DialogTitle>
            ...
            <div className="grid gap-2">
              ...
              <div className="max-h-40 space-y-1 overflow-y-auto rounded-lg border border-border p-1">
                {filteredContacts.length === 0 ? (
                  <p className="px-2 py-1 text-sm text-muted-foreground">{t("noContacts")}</p>
                ) : (
            ...
            <div className="grid gap-4 py-2">
              <div className="flex items-center justify-between rounded-lg border border-border px-4 py-3">
                <span className="text-sm text-muted-foreground">
                  {confirmTarget.contact?.name || confirmTarget.contact?.phone || t("noContact")}
                </span>
              ...
              <div className="grid gap-2">
                ...
                <Select value={confirmDuration} onValueChange={(v) => setConfirmDuration(v ?? "")}>
                  <SelectTrigger className="w-full border-border bg-muted text-foreground">
